...
As of the current controlled reconciliation run, all retained offensive touchdowns have a usable normalized pre-snap distance. The structured ledger exactly matches the first-12-game control for 92 QB, 99 RB, 107 WR, and 120 TE units. The commissioner-approved TE rule is a **flat 12 points for every touchdown**, regardless of distance; QB passing-touchdown credit remains on the standard 6 / 8 / 10 / 12 yard-based tier. Overlapping school-name attribution, suffix-bearing abbreviated names, and verified non-draftable trick plays were reconciled only where cached source evidence was direct. Two conversion events remain unresolved: a UTSA non-quarterback pass with no named receiver and an Air Force conversion with no named rusher. The affected units remain held rather than inferred.

Direct ESPN core review confirms that neither remaining conversion can be resolved from the cached public evidence. UTSA game `401761603` names Devin McCuin as a non-QB passing participant but provides no `patReceiver` participant or receiver name. Air Force game `401760368` explicitly classifies the attempt as a two-point rush, but its participant list contains only the preceding-touchdown rusher/scorer (Liam Szarka) and no conversion scorer. The league therefore retains both as explicit event-level holds rather than crediting a named touchdown scorer by assumption.
...
Game-level reconciliation of the remaining one-touchdown under-count cohort further confirms that no bulk correction is safe. Representative CFBD controls conflict with normalized event evidence: Fresno State has a first-12 QB passing-touchdown control despite no matching offensive passing touchdown in the normalized scoring feed, while Colorado has a TE rushing-touchdown control whose candidate play is attributed to a different named rusher. These cases remain held until their event ownership can be independently reconciled; a season box-score total alone never authorizes a tiered point value.
