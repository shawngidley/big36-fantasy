...
The cached ESPN core feed materially improved K/ST evidence. It supports made-field-goal distances, successful PAT clauses, and ordinary kickoff/punt return touchdowns; 34 K/ST units currently reconcile on those components and have no block or special-teams-safety dependency. A cross-source review of the remaining ESPN core block events found only 90 matches among 140 block events when independently checked against CFBD play text and defensive-team attribution. Therefore, every K/ST total containing a block or special-teams safety remains held until a complete authoritative component control is available.

The ESPN core DEF reconstruction did not produce a publishable defensive total. It recovered many defensive return distances, but none of the 136 units reconciled simultaneously to CFBD first-12-game box controls for sacks, interceptions, defensive touchdowns, and shutouts. Sack counts differed for 134 units, interception counts for 67, and defensive-touchdown counts for 111. All DEF historical point totals therefore remain explicitly held; the available sources are retained only as an audit trail pending a complete defensive event feed or independent component controls.
