...
export default function Standings() {
  ...
  const { overallStandings, divisions, champions } = league.data;
  ...
  return <LeagueShell eyebrow="Season standings"><section className="container pt-10 sm:pt-14">
    ...
    <section className="mt-14 rounded-2xl border border-border bg-card p-6 shadow-sm"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="section-kicker">Championship ledger</p><h2 className="mt-2 font-display text-2xl font-extrabold tracking-tight">Inaugural prize races</h2></div><Badge variant="outline">Ties split equally</Badge></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-xl bg-accent/45 p-4"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">National champion · $1,200</p><p className="mt-2 font-display text-lg font-extrabold">{champions.national.winners.map(winner => winner.teamName).join(" · ") || "Awaiting results"}</p><p className="mt-1 text-xs text-muted-foreground">${champions.national.share.toFixed(2)} per tied winner</p></div><div className="rounded-xl bg-accent/45 p-4"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Conference champions · $200 each</p><p className="mt-2 text-sm font-semibold">{champions.conferences.map(champion => `${champion.conferenceName}: ${champion.winners.map(winner => winner.teamName).join(" / ") || "—"}`).join(" · ")}</p></div><div className="rounded-xl bg-accent/45 p-4"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Position champions · $200 each</p><p className="mt-2 text-sm font-semibold">{champions.positions.map(champion => `${champion.label}: ${champion.winners.map(winner => winner.teamName).join(" / ") || "—"}`).join(" · ")}</p></div></div></section>
  </section></LeagueShell>;
}
