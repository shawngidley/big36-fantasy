...
export default function UnitWeekDetail() {
  ...
  const totalPoints = Math.round(events.reduce((sum, event) => sum + event.computedPoints, 0));
  ...
  return <LeagueShell eyebrow="Position leaders"><section className="container pt-10 sm:pt-14">
    ...
    <div className="mt-6"><p className="section-kicker">{week?.label ?? `Week ${weekNumber}`}</p><h1 className="display-title mt-1">{school} · {position}</h1><p className="mt-2 font-display text-2xl font-extrabold tabular-nums">{totalPoints} pts</p></div>
    <div className="mt-8 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      {events.length ? <div className="divide-y divide-border/70">{events.map(event => <div key={event.id} className="flex items-start justify-between gap-3 px-5 py-3.5">
        ...
        <p className={`shrink-0 font-display text-lg font-extrabold tabular-nums ${event.computedPoints < 0 ? "text-destructive" : ""}`}>{event.computedPoints > 0 ? "+" : ""}{Math.round(event.computedPoints)}</p>
