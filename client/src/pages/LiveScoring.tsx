...
export default function LiveScoring() {
  ...
  const myEvents = myTeam ? recentEvents.filter(event => event.teamName === myTeam.teamName && event.auditAction !== "REVERSAL" && !event.note?.includes("Manual correction: restores points wrongly auto-reversed")) : [];
  ...
  return <LeagueShell eyebrow="Watching your program"><section className="container pt-8 sm:pt-12">
    ...
    {showMine && myTeam ? <>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{positions.map(position => {
        ...
        return <div key={position} className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-center justify-between"><span className="rounded-full border border-border bg-background px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.12em]">{label(position)}</span>{rank ? <span className="text-xs font-bold text-muted-foreground">{rank.tied ? "T" : ""}{rank.rank} of {positionEntries.length}</span> : null}</div><p className="mt-3 font-display text-xl font-extrabold">{pick?.schoolName ?? "Not drafted"}</p>{game ? <p className="mt-1 text-[11px] text-muted-foreground">{game.status === "in_progress" ? <span className="font-bold text-primary">{game.period ? `Q${game.period}` : "Live"} · {game.clock ?? ""}</span> : game.status === "completed" ? "Final" : formatKickoff(game.startDate)}</p> : null}{pick ? <><div className="mt-3 flex items-end justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-muted-foreground">Season</p><p className="font-display text-2xl font-extrabold tabular-nums">{pick.seasonPoints.toFixed(2)}</p></div><div className="text-right"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-muted-foreground">This week</p><p className="font-display text-lg font-extrabold tabular-nums text-primary">{thisWeekPoints > 0 ? "+" : ""}{thisWeekPoints.toFixed(2)}</p></div></div></> : null}</div>;
      })}</div>
