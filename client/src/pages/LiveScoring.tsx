...
export default function LiveScoring() {
  ...
  return <LeagueShell eyebrow="Watching your program"><section className="container pt-8 sm:pt-12">
    ...
    {showMine && myTeam ? <>
      {myDivision ? <MessageBoard divisionId={myDivision.id} divisionName={myDivision.name} canPost isCommissioner={myTeam.isCommissioner} /> : null}

      <div className="mt-8 flex flex-col gap-5 rounded-2xl border border-primary/20 bg-primary/5 p-6 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4"><TeamLogo logoUrl={myTeam.logoUrl} teamName={myTeam.teamName} size="lg" /><div><Link href={`/team/${myTeam.id}`} className="font-display text-2xl font-extrabold hover:text-primary">{myTeam.teamName}</Link><p className="mt-1 text-sm text-muted-foreground">Overall rank #{myRank} of {league.data.owners.length}</p></div></div><div className="flex gap-6"><div className="text-right"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-muted-foreground">Season total</p><p className="mt-1 font-display text-3xl font-extrabold tabular-nums text-primary">{Math.round(myTeam.totalPoints)}</p></div>{weekSummary ? <div className="text-right"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-muted-foreground">This week</p><p className="mt-1 font-display text-3xl font-extrabold tabular-nums">{Math.round(weekSummary.teams.find(t => t.ownerId === myTeam.id)?.points ?? 0)}</p></div> : null}</div></div>

      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div className="flex flex-col gap-3">{positions.map(position => {
