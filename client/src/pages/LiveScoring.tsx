    ...
    {showMine && myTeam ? <>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div className="flex flex-col gap-3">{positions.map(position => {
          ...
          const stats = pick ? <div className="mt-2 flex items-end justify-between border-t border-border/70 pt-2"><div><p className="text-[9px] font-bold uppercase tracking-[.12em] text-muted-foreground">Season</p><p className="font-display text-lg font-extrabold tabular-nums">{pick.seasonPoints.toFixed(2)}</p></div><div className="text-right"><p className="text-[9px] font-bold uppercase tracking-[.12em] text-muted-foreground">This week</p><p className="font-display text-sm font-extrabold tabular-nums text-primary">{thisWeekPoints > 0 ? "+" : ""}{thisWeekPoints.toFixed(2)}</p></div></div> : null;
