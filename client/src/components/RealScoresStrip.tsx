...
const formatKickoffWithDay = (iso: string) => new Date(iso).toLocaleString([], { weekday: "short", hour: "numeric", minute: "2-digit" });
...
export default function RealScoresStrip({ compact = false, scope = "league", ownerId }: { compact?: boolean; scope?: "league" | "all"; ownerId?: string }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
    ...
    {games.length ? (compact ? <div className="grid gap-px bg-border sm:grid-cols-2">{games.map(game => <GameCard key={game.id} game={game} compact />)}</div> : <div>{groupGamesByDay(games).map(group => <div key={group.key}><div className="border-b border-t border-border bg-accent/25 px-5 py-2.5"><p className="text-xs font-bold uppercase tracking-[.1em] text-muted-foreground">{group.label}</p></div><div className="grid gap-px bg-border sm:grid-cols-2 lg:grid-cols-3">{group.games.map(game => <GameCard key={game.id} game={game} />)}</div></div>)}</div>) : <div className="p-8 text-center text-sm text-muted-foreground">{ownerId ? "None of your drafted units are playing this week." : scope === "all" ? "No FBS games scheduled this week." : "No games involving drafted schools this week."}</div>}
  </section>;
}

function GameCard({ game, compact = false }: { game: { id: number; week: number; startDate: string; status: string; period: number | null; clock: string | null; homeTeam: string; awayTeam: string; homePoints: number; awayPoints: number; homeOwners: Array<{ teamName: string; position: string }>; awayOwners: Array<{ teamName: string; position: string }> }; compact?: boolean }) {
  return <Link href={`/scores/${game.week}/${game.id}`} className="block bg-card p-4 transition-colors hover:bg-accent/30">
    <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[.12em] text-muted-foreground"><span className={game.status === "in_progress" ? "text-primary" : ""}>{statusLabel(game.status, game.period, game.clock)}</span>{game.status === "scheduled" ? <span>{compact ? formatKickoffWithDay(game.startDate) : formatKickoff(game.startDate)}</span> : null}</div>
