...
const publicNav = [
  ...
  { label: "Real Scores", path: "/scores" },
  { label: "The Press Box", path: "/press-box" },
    ...
    <header className="league-header sticky top-0 z-40 backdrop-blur-xl">
      <div className="container flex h-[76px] items-center justify-between gap-6">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Primary navigation">
          <Link href="/live" className={cn("mr-1 flex items-center gap-1.5 rounded-full border border-primary/50 bg-primary/15 px-3.5 py-1.5 font-condensed text-base font-bold tracking-wide text-orange-100 transition-colors hover:bg-primary/25", location === "/live" && "bg-primary text-primary-foreground")}><span className="relative flex h-2 w-2"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-orange-400 opacity-75"></span><span className="relative inline-flex h-2 w-2 rounded-full bg-orange-400"></span></span> Live</Link>
          {publicNav.slice(0, 9).map(item => <Link key={item.path} href={item.path} className={cn("league-nav-link rounded-md px-3 py-2 font-condensed text-base font-semibold tracking-wide transition-colors", location === item.path && "league-nav-link-active")}>
            {item.label}
          </Link>)}
          ...
          {publicNav.slice(9).map(item => <Link key={item.path} href={item.path} className={cn("league-nav-link rounded-md px-3 py-2 font-condensed text-base font-semibold tracking-wide transition-colors", location === item.path && "league-nav-link-active")}>
            {item.label}
          </Link>)}
        </nav>
        <div className="flex items-center gap-2">
          ...
          <Sheet><SheetTrigger asChild><Button variant="ghost" size="icon" className="lg:hidden" aria-label="Open navigation"><Menu className="h-5 w-5" /></Button></SheetTrigger><SheetContent side="right" className="w-[290px] overflow-y-auto px-6"><div className="pt-6"><Brand /></div><nav className="mt-10 grid gap-1 pb-8"><Link href="/live" className="mb-2 flex items-center justify-between rounded-lg border border-primary/40 bg-primary/10 px-3 py-3 text-sm font-bold text-primary"><span className="flex items-center gap-2"><span className="relative flex h-2 w-2"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-primary opacity-75"></span><span className="relative inline-flex h-2 w-2 rounded-full bg-primary"></span></span> Live Scoring</span><ChevronRight className="h-4 w-4" /></Link>{publicNav.slice(0, 9).map(item => <Link key={item.path} href={item.path} className="flex items-center justify-between rounded-lg px-3 py-3 text-sm font-semibold hover:bg-accent"><span>{item.label}</span><ChevronRight className="h-4 w-4 text-muted-foreground" /></Link>)}<details className="group my-2 rounded-xl border border-border bg-accent/30 p-2"><summary className="flex cursor-pointer list-none items-center justify-between px-2 py-1.5 font-condensed text-[10px] font-bold uppercase tracking-[.2em] text-muted-foreground [&::-webkit-details-marker]:hidden">Draft <ChevronRight className="h-3.5 w-3.5 transition-transform group-open:rotate-90" /></summary><div className="mt-1">{draftNav.map(item => <Link key={item.path} href={item.path} className="flex items-center justify-between rounded-lg px-2 py-2.5 text-sm font-semibold hover:bg-background"><span>{item.label}</span><ChevronRight className="h-4 w-4 text-muted-foreground" /></Link>)}</div></details>{publicNav.slice(9).map(item => <Link key={item.path} href={item.path} className="flex items-center justify-between rounded-lg px-3 py-3 text-sm font-semibold hover:bg-accent"><span>{item.label}</span><ChevronRight className="h-4 w-4 text-muted-foreground" /></Link>)}<Link href="/commissioner" className="mt-2 flex items-center justify-between rounded-lg bg-primary px-3 py-3 text-sm font-bold text-primary-foreground"><span>Commissioner</span><Shield className="h-4 w-4" /></Link></nav></SheetContent></Sheet>
        </div>
      </div>
    </header>
