    ...
    <header className="league-header sticky top-0 z-40 backdrop-blur-xl">
      <div className="container flex h-[76px] items-center justify-between gap-6">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Primary navigation">
          ...
          {coreNav.map(item => <Link key={item.path} href={item.path} className={cn("league-nav-link rounded-md px-3 py-2 font-condensed text-base font-semibold tracking-wide transition-colors", location === item.path && "league-nav-link-active")}>
            {item.label}
          </Link>)}
          {otherNav.map(item => <Link key={item.path} href={item.path} className={cn("league-nav-link rounded-md px-3 py-2 font-condensed text-base font-semibold tracking-wide transition-colors", location === item.path && "league-nav-link-active")}>
            {item.label}
          </Link>)}
