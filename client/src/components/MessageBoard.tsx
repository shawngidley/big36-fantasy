...
export default function MessageBoard({ divisionId, divisionName, canPost, isCommissioner }: { divisionId: string; divisionName: string; canPost: boolean; isCommissioner?: boolean }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
    ...
    {canPost ? <div className="border-b border-border p-4"><div className="flex items-center gap-2"><input value={draft} onChange={e => setDraft(e.target.value)} onKeyDown={e => { if (e.key === "Enter") submit(); }} maxLength={500} placeholder="Say something to your division..." className="min-w-0 flex-1 rounded-lg border border-border bg-background px-3.5 py-2.5 text-sm outline-none focus:border-primary" /><Button onClick={submit} disabled={!draft.trim() || post.isPending} size="sm">{post.isPending ? "Posting..." : "Post"}</Button></div>{post.error ? <p className="mt-2 text-xs font-semibold text-destructive">{post.error.message}</p> : null}</div> : null}
    {messages.error ? <div className="p-5 text-sm font-semibold text-destructive">Couldn't load messages: {messages.error.message}</div> : messages.data?.length ? <div className="max-h-[420px] divide-y divide-border/70 overflow-y-auto">{messages.data.map(message => <div key={message.id} className="flex items-start gap-3 px-5 py-3.5">
