...
const changes = certification.rows.map(row => {
  ...
  const nextSummary = { ...(current.stat_summary ?? {}), touchdowns: totalTouchdowns, passing_touchdowns: row.official_boxscore.passing_touchdowns, interceptions: row.official_boxscore.interceptions, fumbles_lost: row.official_boxscore.fumbles_lost, qb_stat_line_certified: true, qb_stat_line_source: "CFBD 2025 per-game player box scores · first 12 regular-season games", qb_tier_point_hold: tierPointHold };
  const changed = Number(current.stat_summary?.touchdowns ?? 0) !== totalTouchdowns || Number(current.stat_summary?.passing_touchdowns ?? 0) !== row.official_boxscore.passing_touchdowns || Number(current.stat_summary?.interceptions ?? 0) !== row.official_boxscore.interceptions || Number(current.stat_summary?.fumbles_lost ?? 0) !== row.official_boxscore.fumbles_lost || current.stat_summary?.qb_tier_point_hold !== tierPointHold;
  return { school_name: row.school_name, changed, certified: { touchdowns: totalTouchdowns, passing_touchdowns: row.official_boxscore.passing_touchdowns, interceptions: row.official_boxscore.interceptions, fumbles_lost: row.official_boxscore.fumbles_lost }, next_summary: nextSummary };
});
if (apply) {
