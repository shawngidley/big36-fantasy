...
The deployed one-minute Heartbeat task was first verified in its safe disabled state, then enabled after the Tier 4 live-data entitlement was confirmed. Its protected `/api/scheduled/gameday-refresh` callback now returns successful `outside-gameday-window` responses outside approved Thursday–Sunday FBS windows, confirming the project-level scheduler, cron authentication, durable task binding, and request-conserving guard are working before the first active-game import.

After a later checkpoint briefly captured the root page while its query was pending, the public home was opened directly once the successful `/api/trpc/league.snapshot` response had returned. The full 36 Football home content rendered normally, including the inaugural hero, standings and research navigation, national-race state, program/conference totals, and live-feed summary. This confirms the transient ledger-loading screenshot was a capture-timing race, not a persistent home-page defect.

The 2025 K/ST catalog was audited after an inflated special-teams leaderboard appeared. The cause was a classifier that interpreted the parenthetical kicker notation attached to ordinary offensive touchdown descriptions as a K/ST touchdown. The scorer now uses the provider’s play type for return and blocked-kick touchdowns, while separately retaining the one-point made PAT. The rebuilt catalog has realistic K/ST event totals—for example, Oklahoma recorded 23 field goals, 25 PATs, and one blocked field goal for 214 points over 12 games—and the public research page rendered normally after the correction.

The canonical `https://36football.com` deployment was opened after cache propagation. It served the full public application with the verified browser title **36 Football — Inaugural Season**, confirming that the official metadata update reached the custom domain.
