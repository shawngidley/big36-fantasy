...
- [x] Review whether held 2025 research totals should instead be displayed as explicitly qualified estimates, and implement only if the commissioner accepts the resulting accuracy tradeoff. (Reviewed live records with the commissioner: he wants the existing published totals to lead the board, not newly inferred estimates for rows whose point fields are truly null. The board now follows that policy.)
...
- [x] Send one commissioner-approved non-draft Twilio SMS test to the registered commissioner phone number and record the provider response without altering draft state. (Twilio queued the test message as `SMf2eeb9462bc7c2c3dd497d07672bd45f`; no draft data changed.)
- [x] Update the Twilio sender configuration to the confirmed SMS-capable number +1 216-600-0866 before retrying the approved test. (The configured number is now verified as owned by the account and SMS-capable.)
- [x] Fix the My Draft queue action mapping so remove and reorder calls use persisted queue-entry UUIDs rather than composite school-position keys. (The live error originated in the queue-add audit entry; it now records the inserted queue row’s UUID.)
- [x] Prefix each existing ten-digit owner registration phone number with +1 while preserving already normalized +1 numbers, then enforce the same U.S. E.164 normalization on future registration and profile updates. (All 14 existing registration numbers were backfilled; future inputs normalize to U.S. +1 E.164.)
- [x] Fix owner-profile updates so registration-only inspiration data is retained in b36_owner_registrations but is never written to b36_owners, whose schema has no inspiration column.
- [x] Refine upcoming-pick SMS wording to state that the owner ahead is on the clock, highlight the full ten-minute preparation window, and add a concise football identifier that remains reliable in standard SMS. (The message now leads with 🏈 36 Football — ON DECK, names the active and upcoming pick numbers, and explicitly states that the owner has the current ten-minute clock to prepare.)
- [x] Confirm and clearly explain the missed-pick policy: whether a queue auto-picks or the owner retains an open roster slot for a later self-service selection. (Confirmed: missed turns are marked SKIPPED, no queue auto-pick occurs, and the owner may later fill that open roster slot with any available unit during the active draft window.)
- [x] Let an on-clock owner select and submit an eligible unit directly from the available-unit panel, while retaining the queue and manual school-entry methods. (On-clock cards now show Draft unit, open a confirmation dialog, and use the same protected selection procedure as the manual form.)
- [x] Design a commissioner-controlled, auditable 36-team draft lottery that randomly reveals the order from pick 36 through pick 1 before the inaugural draft, and obtain approval before implementation. (Commissioner approved the locked-timeline approach.)
- [x] Build the approved public draft lottery with commissioner start/pause/abort controls, cryptographically generated locked order, and automatic reveals from Position 36 to Position 1 every 20 seconds. (Implemented as a server-authoritative locked timeline with durable state and commissioner operations.)
- [x] Build the approved public draft lottery with commissioner start/pause/abort controls, cryptographically generated locked order, and automatic reveals from Position 36 to Position 1 every 20 seconds. (Server-persisted lottery state, immutable commitment, transactional reveal synchronizer, audit events, and final serpentine-turn generation are implemented.)
- [x] Design a dramatic broadcast-style lottery reveal page with a countdown, live reveal stage, completed-order rail, program branding, and reduced-motion accessibility support. (Available at `/draft-lottery` with public polling and a commissioner Draft Lottery control tab.)
- [x] Run a commissioner-supervised full 36-program lottery rehearsal, verify all reverse reveals and generated serpentine turns, then reset the rehearsal without retaining its draft order. (Rollback-safe rehearsal reached COMPLETE with 36 unique reverse reveals, 216 unique serpentine picks, and all 36 programs represented; no rehearsal data persisted.)
- [x] Display Sunday, August 23 at 9:00 PM Eastern as the planned live lottery time while requiring an explicit commissioner Start action before any lottery order is generated or revealed. (Schedule is stored as 2026-08-24 01:00 UTC / 9:00 PM Eastern; no live lottery record exists until a commissioner selects Start.)
- [x] Consolidate Lottery, My Draft, and Draft Order under an accessible Draft navigation dropdown on desktop and a grouped Draft section on mobile. (Desktop uses a keyboard-accessible disclosure menu; mobile groups the same three destinations under Draft.)
- [x] Verify the end-to-end draft selection write path assigns each selected school-position unit to the correct owner team, roster slot, and downstream scoring identity. (Verified live selection function, owner/slot joins, and 32 related regressions: a pick is inserted with the active or skipped owner’s ID, linked to that turn, and all standings/scoring reads resolve from the slot’s owner ID.)
- [x] Restore Josh Tolchinsky’s accidentally declined St. Margaret’s School for Wayward Girls · Weasels registration to approved status and assign it to an available program slot. (Approved and assigned to the former Team 3 placeholder slot; program identity, Weasels nickname, logo, colors, and owner email were preserved.)
- [x] Remove the commissioner-access explanatory sentence from the owner sign-in page without changing normal owner or commissioner authentication. (Sentence removed; sign-in regressions and TypeScript validation pass.)
- [x] Verify and explain the commissioner workflow for assigning approved owner programs to the six league conferences. (Commish → Programs → Edit opens the selected program editor; the Conference dropdown supports Unassigned or any of the six conferences, and Save program applies the assignment while enforcing the six-program conference cap.)
- [x] Repair the nonresponsive commissioner Programs edit action so team details and conference assignments can be changed. (Edit reliably populates the form, then scrolls and focuses the program editor so the commissioner sees the active form immediately, including on mobile.)
- [x] Add a public Scoring page that accurately explains the active 36 Football scoring system by position and add it to league navigation. (Published at `/scoring`; verified on desktop and mobile against the authoritative Year 1 scoring rules.)
- [x] Build a public 2026 college-football research hub with curated links for player news, team depth charts, schedules, rankings, statistics, recruiting, and general season learning. (Published at `/college-football-guide` with ten curated resources, a draft-research workflow, and responsive desktop/mobile presentation.)
- [x] Keep the Twilio live credential regression informative when sandbox outbound access returns HTTP 451, while still failing for true Twilio authentication or sender-configuration errors. (The test explicitly tolerates only the sandbox’s documented 451 restriction and retains full authentication/sender checks whenever Twilio is reachable.)
- [x] Move the Commissioner destination to the top of the mobile navigation menu while preserving the grouped Draft section and all existing links. (Commissioner is now the first mobile menu action, above Join League and the grouped Draft links.)
- [x] Verify the latest published mobile navigation update is serving on 36football.com and resolve any client-cache or deployment visibility issue. (Confirmed on the public domain: the opened mobile drawer lists Commissioner immediately after the brand and before Join League.)
- [x] Determine whether the project directly uses Supabase analytics/endpoints/logs.all and migrate to analytics/endpoints/logs if required before September 23, 2026. (No direct use exists in project source or configuration; the only site analytics script points to the separate Manus-hosted Umami endpoint, so no migration is required.)
- [x] Confirm Supabase’s current account-to-account project transfer options and document the safe migration implications for the 36 Football database and application credentials. (Direct project transfer between organizations is supported and preferred; no fresh database migration is needed if prerequisites are met. Full assessment saved at `research/supabase_project_transfer_assessment.md`.)
- [x] Back up the current 36 Football project to GitHub, including the latest published code and research documentation, and verify the repository state. (Current project checkpoint synchronized to the connected `shawngidley/big36-fantasy` repository.)
- [x] Keep the connected GitHub repository current after every saved 36 Football project checkpoint and confirm the initial repository backup is synchronized now. (The project’s connected `user_github` remote is configured to synchronize as part of each saved checkpoint.)
- [x] Add a site-wide footer reading “© 2026 36 Football. All rights reserved. Website designed and developed by Ascend CX.” with links to Ascend CX, Terms of Use, and Privacy Policy. (The Ascend CX credit links to ascend-cx.com and the legal links are live.)
- [x] Publish 36 Football Terms of Use adapted from the commissioner-provided content, with Ascend CX as the site developer and licensing contact. (Published at `/terms`.)
- [x] Publish a 36 Football Privacy Policy covering league SMS notifications, opt-out language, data handling, and the contact email hello@ascend-cx.com. (Published at `/privacy`.)
- [x] Verify the public 36football.com build exposes the new footer, Terms of Use, and Privacy Policy, and resolve any client-cache or deployment visibility issue. (Confirmed live on 36football.com: the footer includes Ascend CX, Terms of Use, and Privacy Policy links.)
- [x] Decide and, if approved, add the appropriate Matt Janssen league-founder attribution to 36 Football public legal or site-credit content without placing it in the Privacy Policy unnecessarily. (Approved and added to the Terms of Use Ownership section; the Privacy Policy remains focused on data practices.)
- [x] Verify the connected 36 Football GitHub repository is ready for the user’s Claude Code workflow and document the safe access steps without disrupting checkpoint synchronization. (The private `shawngidley/big36-fantasy` repository is on `main`; Claude Code running locally uses the user’s existing GitHub identity, so no ownership transfer or new collaborator is required. Pull before editing, commit and push code changes, and keep secrets and production database migrations outside normal Git commits.)
