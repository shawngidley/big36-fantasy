...
describe("36 Football automatic scoring map", () => {
  ...
  it("does NOT credit a defensive touchdown for an interception return that was overturned on replay review, even though CFBD's play text still embeds the reversed original call's 'TOUCHDOWN' wording - real Ole Miss/Charlotte play: an interception originally ruled a 76-yard pick-six was overturned to a 38-yard return only, but the final play text keeps the whole '(Original Play: ... TOUCHDOWN, clock 00:04)' parenthetical from the reversed call, wrongly crediting Ole Miss's DST 12 points for a touchdown that officially never happened (confirmed: no defensive/special-teams touchdown appears anywhere in the real box score for this 41-9 game)", () => {
    const play = { id: 401856676364, gameId: 401856676, offense: "Charlotte", defense: "Ole Miss", scoring: false, playType: "Pass Interception Return", playText: "(00:17) Shotgun #9 C.Gonzales pass intercepted by #29 N.Cull at OM24 #29 N.Cull return 38 yards to the CLT38, out of bounds at CLT38. The previous play is under automatic review - \"Runner was out of bounds\". CALL OVERTURNED. (Original Play: (00:17) Shotgun #9 C.Gonzales pass intercepted by #29 N.Cull at OM24 QB hurried by #3 B.Purchase #29 N.Cull return 76 yards to the CLT00 TOUCHDOWN, clock 00:04)" };
    const candidates = mapLivePlayToCandidates({ play, stats: [], roster: [], selectedSchoolPositions: [{ schoolName: "Ole Miss", position: "DST" }] });
    expect(candidates.some(candidate => candidate.eventType === "DEFENSIVE_TOUCHDOWN")).toBe(false);
    expect(candidates.some(candidate => candidate.eventType === "DEFENSIVE_TURNOVER")).toBe(true);
  });
